...
if (saisie === null) {
    ...
    if (isNaN(score)) {
        ...
        if (score === 100) {
            ...
            rang    = "A — Expert";
            message = "Excellent travail, continue.";
        } else if (score >= 55) {
            rang    = "B — Confirmé";
            message = "Bon niveau. Encore un effort.";
        } else if (score >= 40) {
...
if (saisieNiveau === null || saisieAdmin === null || code === null) {
    ...
    if (isNaN(niveau) || niveau < 1 || niveau > 5) {
        ...
        const codeCorrect = code === CODE_VALIDE;
        const estBanni    = niveau === 3 && !estAdmin;
        ...
        if (estAdmin && codeCorrect) {
            ...
            afficherOk("✅ ACCÈS ZONE SÉCURISÉE", "Zones A et B");
        } else if (niveau >= 2 && niveau < 4 && !estBanni) {
    ...
    switch (choix.trim()) {
        ...
        case "3":
            if (or >= 10) {
                ...
                afficherErreur(\`Or insuffisant ! Tu as \${or} or (besoin : 10).\`);
            }
            break;
