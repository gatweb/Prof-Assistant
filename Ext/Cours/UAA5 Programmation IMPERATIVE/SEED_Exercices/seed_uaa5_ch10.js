...
function jouer(choixJoueur) {
    ...
    if (choixJoueur === choixOrdi) {
        resultat = "Égalité 🤝"; classe = "card card-draw"; egalites++;
    } else if (
        (choixJoueur === "pierre"  && choixOrdi === "ciseaux") ||
        (choixJoueur === "ciseaux" && choixOrdi === "feuille") ||
        (choixJoueur === "feuille" && choixOrdi === "pierre")
    ) {
        resultat = "Gagné 🎉"; classe = "card card-ok"; victories++;
    } else {
        resultat = "Perdu 😞"; classe = "card card-bad"; defaites++;
    }
...
document.getElementById("btnReset").addEventListener("click", function() {
    victories = 0; defaites = 0; egalites = 0;
...
document.getElementById("btnAjouter").addEventListener("click", function() {
    ...
    if (isNaN(note)  || note  < 0 || note > 20) {
        alert("Note invalide (0–20)."); return;
    }
    if (isNaN(coeff) || !Number.isInteger(coeff) || coeff < 1) {
        alert("Coefficient invalide (entier ≥ 1)."); return;
    }
...
document.getElementById("btnCalculer").addEventListener("click", function() {
    ...
    if      (moyenne >= 14) { mention = "🏆 Distinction";       classeCard = "card-ok";  }
    else if (moyenne >= 10) { mention = "✅ Réussite";           classeCard = "card-ok";  }
    else if (moyenne >= 8)  { mention = "⚠️ Ajournement";       classeCard = "card-warn"; }
    else                    { mention = "❌ Échec";              classeCard = "card-bad";  }
...
document.getElementById("btnReset").addEventListener("click", function() {
    sommeNoteCoeff = 0; sommeCoeffs = 0; nbNotes = 0;
