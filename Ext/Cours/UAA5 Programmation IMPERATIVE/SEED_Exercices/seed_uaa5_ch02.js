...
\`\`\`javascript
const nomPersonnage = "Shadow_X";
const classe = "Mage";
const niveau = 23;
let hp = 340;
const estVivant = true;
...
console.log(typeof nomPersonnage);  // "string"
console.log(typeof niveau);         // "number"
console.log(typeof estVivant);      // "boolean"
\`\`\`
            `.trim()
        }
    },
...
const degatsBase = force * 2.5;
const vitesse = agilite % 10 + 5;
const puissanceMagique = intelligence ** 2 / 10;
const hpMax = (force + agilite) * niveau;
const armure = Math.floor(agilite / 3);
...
Analyse ses prédictions et explique pourquoi celles qui sont fausses le sont, en insistant sur la distinction entre + (ambigu avec les strings) et les autres opérateurs (toujours numériques).`,
            ...
            niveau_3_md: `
🛠️ **Résultats corrects et explications :**

\`\`\`javascript
console.log("5" + 3);        // "53"  — + avec string = concaténation
console.log("5" - 3);        // 2     — - force conversion en nombre
console.log("5" * "2");      // 10    — * force conversion en nombre
console.log("abc" - 1);      // NaN   — impossible à convertir
console.log(true + 1);       // 2     — true vaut 1 en JS
console.log(false + 10);     // 10    — false vaut 0 en JS
console.log(null + 5);       // 5     — null vaut 0
console.log(undefined + 5);  // NaN   — undefined ne se convertit pas

// Corrections avec Number() :
console.log(Number(true) + 1);       // 2
console.log(Number(null) + 5);       // 5
console.log(Number(undefined) + 5);  // NaN (on ne peut pas faire mieux)
\`\`\`
            `.trim()
        }
    }
