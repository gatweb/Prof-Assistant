...
const EXERCICES = [
    {
        ...
        lien_cours: "https://docs.google.com/document/d/1rJJfI5x-MPx0KnBJ9SnkCy6jfZ683cU7pFh12hrzoZA/edit?usp=sharing",
        enonce_md: `
...
function titreSec(t) {
    const d = document.className = "titre-bloc";
    d.textContent = t;
    document.getElementById("output").appendChild(d);
}
...
Identifie le premier problème et guide avec une question ciblée.`,
            ...
            niveau_3_md: `
🛠️ **Squelette de traduction pseudo-code → JS attendu :**
 
Voici comment structurer ton code de vérification du nombre premier. Complète les parties avec \`...\` :
...
document.getElementById("btnVerifier").addEventListener("click", function() {
    ...
    // Borne optimisée : Math.floor(Math.sqrt(n))
    for (let i = 2; i <= Math.floor(Math.sqrt(n)); i++) {
    ...
    if (estPremier) {
        ...
        // Trouver et afficher tous les diviseurs (deuxième boucle complète de 1 à n)
        titreSec("Diviseurs :");
        for (let i = 1; i <= n; i++) {
            if (n % i === 0) {
                afficher(i);
...
Demande à l'élève quel bug il a trouvé. S'il a tout corrigé, vérifie que les commentaires "BUG CORRIGÉ" sont présents et que l'amélioration (% réussite) est ajoutée.`,
            ...
            niveau_3_md: `
🛠️ **Les 4 corrections logiques + amélioration attendues :**
 
Voici comment corriger les 4 bugs et ajouter l'affichage du pourcentage de réussite. Complète les morceaux avec \`...\` :
 
\`\`\`javascript
// Bouton "Ajouter" — Bug 1 (validation inversée)
document.getElementById("btnAjouter").addEventListener("click", function() {
    const note = Number(document.getElementById("inputNote").value);
 
    // BUG CORRIGÉ 1 : la condition d'origine note >= 0 && note <= 20 rejetait par erreur les notes valides !
    // On veut lever une erreur si la note est en dehors des bornes, ou n'est pas un nombre :
...
// Bouton "Calculer" — Bugs 2 (min à 0), 3 (moyenne dans la boucle) et 4 (affectation au lieu de comparaison)
document.getElementById("btnCalculer").addEventListener("click", function() {
    document.getElementById("output").innerHTML = "";
    ...
    let somme    = 0;
    let min      = null;  // BUG CORRIGÉ 2 : initialiser min à null pour éviter que 0 reste indéfiniment le min si toutes les notes sont > 0
    let max      = null;
    let reussies = 0;
    ...
    for (let i = 0; i < notes.length; i++) {
        ...
        // BUG CORRIGÉ 3 : le calcul moyenne = somme / notes.length doit être retiré de la boucle pour n'être exécuté qu'à la toute fin !
 
        if (min === null || note < min) min = note;
        if (max === null || note > max) max = note;
 
        // BUG CORRIGÉ 4 : l'opérateur '=' est une affectation. Pour comparer, on utilise '>=' :
        if (note >= 10) {
            reussies++;
        }
    }
 
    // Calcul final de la moyenne (après la boucle)
    ...
    // AMÉLIORATION : calcul et affichage du pourcentage de réussite
