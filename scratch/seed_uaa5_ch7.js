...
const EXERCICES = [
    {
        ...
        lien_cours: "https://docs.google.com/document/d/1HMj5LcYU0mOVaQdxRxYfRkjWeDehoCB-Jg1S7w-aq4Q/edit?usp=sharing",
        enonce_md: `
...
Identifie le premier problème et guide avec une question ciblée.`,
            ...
            niveau_3_md: `
🛠️ **Squelette logique d'accumulation attendu :**
 
Voici l'organisation de la boucle de traitement de chaque partie du tableau de chaînes de caractères. Remplis les \`...\` :
...
for (let i = 0; i < parties.length; i++) {
    ...
    // Validation : si ce n'est pas un nombre, on incrémente nbIgnores et on saute au tour suivant
    if (isNaN(n)) { 
        nbIgnores++; 
        continue; 
    }
    ...
    // Calcul du minimum et du maximum (comparaisons avec null)
    if (min === null || n < min) min = n;
    if (max === null || n > max) max = n;
    
    // Répartition selon le signe du nombre
    if (n > 0) { 
        nbPositifs++; 
        sommePositifs += n; 
    } else if (n < 0) { 
...
// Affichage DOM dynamique après la boucle
...
> ⚠️ **Attention au Piège :** Ne pas initialiser \`min\` et \`max\` à \`0\`. Si tous les nombres de l'élève sont négatifs (ex: -5, -12, -8), \`0\` resterait par erreur le maximum ! C'est pour cela qu'on utilise \`null\` au départ.
            `.trim()
        }
    },
...
Identifie le premier problème visible et guide avec une question.`,
            ...
            niveau_3_md: `
🛠️ **Squelette de structures de boucles imbriquées attendu :**
 
Voici l'organisation de ta boucle extérieure de lignes combinée aux boucles intérieures par type de pyramide. Remplis les \`...\` :
...
document.getElementById("btnGenerer").addEventListener("click", function() {
    ...
    const car = carSaisi[0];   // On prend uniquement le 1er caractère
    ...
    for (let row = 1; row <= hauteur; row++) {
        ...
        if (styleChoisi === "plein") {
            // Boucle intérieure : ajoute row fois le caractère
            for (let col = 1; col <= row; col++) {
                ligne += car;
            }
        } else if (styleChoisi === "creux") {
            // Boucle intérieure : n'affiche le caractère que sur les bords du triangle
            for (let col = 1; col <= row; col++) {
                if (col === 1 || col === row || row === hauteur) {
                    ligne += car;
                } else {
                    ligne += " "; // Espace intérieur
                }
            }
        } else if (styleChoisi === "centre") {
            // 1. Répéter les espaces de gauche
            ligne = " ".repeat(hauteur - row);
            // 2. Boucle intérieure de caractères centrés (formule : 2 * row - 1)
            for (let col = 1; col <= 2 * row - 1; col++) {
                ligne += car;
            }
        }
 
        // Création et insertion de la ligne dans le DOM
        const div = document.createElement("div");
        div.className = "ligne";
        div.textContent = ligne;
        output.appendChild(div);
