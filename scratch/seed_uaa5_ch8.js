...
const EXERCICES = [
    {
        ...
        lien_cours: "https://docs.google.com/document/d/1j_4Hsx5jNIYo0JDAZbXVTF2kRfbHdsuXWAz4triWBcg/edit?usp=sharing",
        enonce_md: `
...
Identifie le premier problème (état mal placé dans le listener, disabled oublié, réinitialisation incomplète, comparaison == au lieu de ===...) et pose une question ciblée.`,
            ...
            niveau_3_md: `
🛠️ **Squelette logique du Jeu de Devinette attendu :**
 
Voici comment ordonner ton état persistant global (en dehors des listeners) et la logique d'évaluation au clic. Remplis les \`...\` :
 
\`\`\`javascript
// État persistant du jeu — Declarés en dehors pour survivre d'un clic à l'autre
let secret     = Math.floor(Math.random() * 100) + 1;
let tentatives = 0;
let termine    = false;
...
// Écouteur d'événements du bouton Essayer
document.getElementById("btnEssayer").addEventListener("click", function() {
    // 1. Si la partie est terminée, on ignore le clic
    if (termine) return;
 
    const essai = Number(document.getElementById("inputEssai").value);
 
    // 2. Validation stricte de la saisie
    if (isNaN(essai) || essai < 1 || essai > 100) {
...
// Écouteur d'événements du bouton Rejouer
document.getElementById("btnRejouer").addEventListener("click", function() {
    // Régénérer un nombre et réinitialiser l'état
    secret     = Math.floor(Math.random() * 100) + 1;
    tentatives = 0;
    termine    = false;
...
### Calculate une différence en jours
...
Identifie le premier problème et pose une question ciblée.`,
            ...
            niveau_3_md: `
🛠️ **Squelette logique du Dashboard Temporel attendu :**
 
Voici comment formater et extraire les valeurs temporelles de l'objet \`Date\` au clic sur le bouton Actualiser. Remplis les \`...\` :
...
document.getElementById("btnActualiser").addEventListener("click", function() {
    ...
    // 1. Extraction des composantes temporelles
    const annee    = maintenant.getFullYear();
    const mois     = maintenant.getMonth();      // Rappel : index de 0 à 11
    const jour     = maintenant.getDate();
    const jourSem  = maintenant.getDay();        // Rappel : index de 0 (dimanche) à 6 (samedi)
    const heures   = maintenant.getHours();
    const minutes  = maintenant.getMinutes();
    const secondes = maintenant.getSeconds();
 
    // 2. Formatage de l'heure avec padStart(2, "0")
    const p = n => String(n).padStart(2, "0");
    const heureStr = \`\${p(heures)}:\${p(minutes)}:\${p(secondes)}\`;
    document.getElementById("heureAffichage").textContent = heureStr;
 
    // 3. Salutation contextuelle selon l'heure
    let salutation;
    if (heures < 12) {
        salutation = "🌅 Bonjour !";
    } else if (heures < 18) {
    ...
    // 4. Construction de la date complète en français
    const dateComplete = \`\${JOURS[jourSem]} \${jour} \${MOIS[mois]} \${annee}\`;
    
    // 5. Calcul des jours restants avant Noël (25 décembre)
    const noel         = new Date(annee, 11, 25); // 11 = Décembre
    const diffJours    = Math.ceil((noel - maintenant) / (1000 * 60 * 60 * 24));
