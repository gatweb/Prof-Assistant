...
const EXERCICES = [
    {
        ...
        lien_cours: "https://docs.google.com/document/d/1finFAVk1pGcb2BG-7qDKZs-If5tTYJvMhsw8Q_4pDHc/edit?usp=sharing",
        enonce_md: `
...
Voici l'organisation attendue pour ton écouteur d'événement. Remplis les \`...\` :
...
document.getElementById("btnGenerer").addEventListener("click", function() {
    ...
    // Validation stricte du nombre saisi
    if (isNaN(n) || n < 1 || n > 20) {
        const err = document.createElement("div");
        err.className = "erreur";
        err.textContent = "Nombre invalide — entre 1 et 20.";
        output.appendChild(err);
        return; // Arrêt immédiat de la fonction
    }
 
    // Génération de la table via la boucle for
    for (let i = 1; i <= 10; i++) {
        const div = document.createElement("div");
        div.className = "item";
        // Injection dynamique de la formule de multiplication dans le HTML
...
Identifie le premier problème et pose une question ciblée sans donner le code.`,
            ...
            niveau_3_md: `
🛠️ **Squelette logique d'analyse de caractères attendu :**
 
Voici la boucle de parcours de texte et les compteurs d'accumulateurs. Complète les morceaux avec \`...\` :
...
document.getElementById("btnAnalyser").addEventListener("click", function() {
    ...
    // Fonction utilitaire pour générer une ligne de statistiques dans le DOM
...
Identifie le problème principal et pose une question ciblée.`,
            ...
            niveau_3_md: `
🛠️ **Squelette structurel FizzBuzz attendu :**
 
Voici l'architecture de la boucle et le bon ordre d'évaluation des modulos (FizzBuzz en premier). Complète les parties vides \`...\` :
...
> 💡 **Le savais-tu ?** L'ordre des vérifications est le piège n°1 de cet exercice dans les tests de recrutement. Si on vérifie la division par \`3\` avant la division par \`15\`, le nombre 15 affichera "Fizz" et n'atteindra jamais la condition "FizzBuzz".
            `.trim()
        }
    }
];
